...
jar/exe/dmg package, or run "java -jar sbr-client.jar" to start the
installer. See https://bitbucket.org/JeanLucPicard/sbr/issues/283 for more
details about the IzPack installer. After installation, use the shortcuts or
desktop icons to start the Sbr server.


Using the sbr-client.zip package:

Unpack the sbr-client.zip package and open a shell in the resulting sbr
...
the sbr_db blockchain database. The start.sh script can be run from any
directory, it starts the java process in the background and uses ~/.sbr for
configuration files, logs, and the sbr_db database. Unlike run.sh, start.sh
...
message "Sbr server 1.x.x started successfully" in the console log. If run in
...
where the Sbr UI should now be available.
...
to the Sbr installation directory, as the use of special characters may result
...
conf/sbr-default.properties file. All possible settings are listed, with
detailed explanation. If you decide to change any setting, do not edit
sbr-default.properties directly, but create a new conf/sbr.properties file
and only add to it the properties that need to be different from the default
values. You do not need to delete the defaults from sbr-default.properties, the
settings in sbr.properties override those in sbr-default.properties. This way,
when upgrading the software, you can safely overwrite sbr-default.properties
with the updated file from the new package, while your customizations remain
safe in the sbr.properties file.


How to contribute?

There are many ways to contribute to Sbr. Here are some examples:
...
The Sbr software is a client-server application. It consists of a java server
...
requests which other Sbr client applications could use.

The blockchain is stored on disk using the H2 embedded database, inside the
sbr_db directory. When upgrading, you should not delete the old sbr_db
...
sbr_db, except that it will take some extra time to download the blockchain
from scratch.

The default Sbr client does not store any wallet-type file on disk. Unlike
...
which you see in the console window, but also to a file sbr.log, which gets
overwritten at restart. In case of an error, the sbr.log file may contain
...
sbr.jar file using the enclosed jar.sh script, and then sbr.jar should be
