...
echo "compiling SBR core..."
find src/java/sbr/ -name "*.java" > sources.tmp
javac -encoding utf8 -sourcepath "${SP}" -classpath "${CP}" -d classes/ @sources.tmp || exit 1
echo "SBR core class files compiled successfully"

echo "compiling SBR desktop..."
find src/java/sbrdesktop/ -name "*.java" > sources.tmp
javac -encoding utf8 -sourcepath "${SP}" -classpath "${CP}" -d classes/ @sources.tmp
if [ $? -eq 0 ]; then
    echo "SBR desktop class files compiled successfully"
else
    echo "if javafx is not supported, SBR desktop compile errors are safe to ignore, but desktop wallet will not be available"
fi
