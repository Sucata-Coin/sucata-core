...
final class BlockImpl implements Block {
    ...
    static BlockImpl parseBlock(JSONObject blockData) throws SbrException.NotValidException {
        try {
            ...
            if (!block.checkSignature()) {
                throw new SbrException.NotValidException("Invalid block signature");
            }
            return block;
    ...
    boolean verifyGenerationSignature() throws BlockchainProcessor.BlockOutOfOrderException {
        ...
        try {
            ...
            long effectiveBalance = account == null ? 0 : account.getEffectiveBalanceSBR();
    ...
    void apply() {
        ...
        if (this.height > 3) {
            ...
            for (int i = 0; i < backFees.length; i++) {
                ...
                Logger.logDebugMessage("Back fees %f %s to forger at height %d", ((double)backFees[i])/Constants.ONE_SBR, Constants.COIN_SYMBOL, this.height - i - 1);
        ...
        if (totalBackFees != 0) {
            Logger.logDebugMessage("Fee reduced by %f %s at height %d", ((double)totalBackFees)/Constants.ONE_SBR, Constants.COIN_SYMBOL, this.height);
        }
