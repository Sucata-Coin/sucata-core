...
/**
 * Handle logging for the Sbr node server
 */
public final class Logger {
     ...
     * The sbr/conf/logging-default.properties and sbr/conf/logging.properties configuration
    ...
    static {
        ...
        System.setProperty("java.util.logging.manager", "sbr.util.SbrLogManager");
        if (!(LogManager.getLogManager() instanceof SbrLogManager)) {
            System.setProperty("java.util.logging.manager",
                    (oldManager != null ? oldManager : "java.util.logging.LogManager"));
        }
        if (! Boolean.getBoolean("sbr.doNotConfigureLogging")) {
            try {
                Properties loggingProperties = new Properties();
                Sbr.loadProperties(loggingProperties, "logging-default.properties", true);
                Sbr.loadProperties(loggingProperties, "logging.properties", false);
                Sbr.updateLogFileHandler(loggingProperties);
        ...
        log = org.slf4j.LoggerFactory.getLogger(sbr.Sbr.class);
        enableStackTraces = Sbr.getBooleanProperty("sbr.enableStackTraces");
        enableLogTraceback = Sbr.getBooleanProperty("sbr.enableLogTraceback");
    ...
    public static void shutdown() {
        if (LogManager.getLogManager() instanceof SbrLogManager) {
            ((SbrLogManager) LogManager.getLogManager()).sbrShutdown();
        }
    }
