...
public abstract class EntityDbTable<T> extends DerivedDbTable {
    ...
    public void checkAvailable(int height) {
        if (multiversion) {
            int minRollBackHeight = isPersistent() && Sbr.getBlockchainProcessor().isScanning() ?
                    Math.max(Sbr.getBlockchainProcessor().getInitialScanHeight() - Constants.MAX_ROLLBACK, 0)
                    : Sbr.getBlockchainProcessor().getMinRollbackHeight();
    ...
    private boolean doesNotExceed(int height) {
        return Sbr.getBlockchain().getHeight() <= height && ! (isPersistent() && Sbr.getBlockchainProcessor().isScanning());
    }
