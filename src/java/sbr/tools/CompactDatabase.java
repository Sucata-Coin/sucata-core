 ...
 *
 *   java -cp "classes:lib/*:conf" sbr.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Dsbr.runtime.mode=desktop sbr.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    public static void main(String[] args) {
        //
        // Initialize Sbr properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "sbr.testDb" : "sbr.db";
        String dbType = Sbr.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = Sbr.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = Sbr.getDbDir(Sbr.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = Sbr.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = Sbr.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = Sbr.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "sbr.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "sbr.mv.db");
            if (!dbFile.exists()) {
