...
public abstract class MonetarySystem extends TransactionType {
    ...
    public static final TransactionType CURRENCY_ISSUANCE = new MonetarySystem() {
        ...
        private final Fee FIVE_LETTER_CURRENCY_ISSUANCE_FEE = new Fee.ConstantFee(40 * Constants.ONE_SBR);
        private final Fee FOUR_LETTER_CURRENCY_ISSUANCE_FEE = new Fee.ConstantFee(1000 * Constants.ONE_SBR);
        private final Fee THREE_LETTER_CURRENCY_ISSUANCE_FEE = new Fee.ConstantFee(25000 * Constants.ONE_SBR);
        ...
        @Override
        Attachment.MonetarySystemCurrencyIssuance parseAttachment(ByteBuffer buffer) throws SbrException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws SbrException.ValidationException {
            ...
            if (attachment.getMaxSupply() > Constants.MAX_CURRENCY_TOTAL_SUPPLY
                ...
                throw new SbrException.NotValidException("Invalid currency issuance: " + attachment.getJSONObject());
            }
            int t = 1;
            for (int i = 0; i < 32; i++) {
                if ((t & attachment.getType()) != 0 && CurrencyType.get(t) == null) {
                    throw new SbrException.NotValidException("Invalid currency type: " + attachment.getType());
                }
                t <<= 1;
            }
    ...
    public static final TransactionType RESERVE_INCREASE = new MonetarySystem() {
        ...
        void validateAttachment(Transaction transaction) throws SbrException.ValidationException {
            ...
            if (attachment.getAmountPerUnitNQT() <= 0) {
                throw new SbrException.NotValidException("Reserve increase amount must be positive: " + attachment.getAmountPerUnitNQT());
            }
        ...
        void undoAttachmentUnconfirmed(Transaction transaction, Account senderAccount) {
            ...
            if (currency != null) {
                ...
                Transaction currencyIssuance = Sbr.getBlockchain().getTransaction(attachment.getCurrencyId());
    ...
    public static final TransactionType RESERVE_CLAIM = new MonetarySystem() {
        ...
        void validateAttachment(Transaction transaction) throws SbrException.ValidationException {
            ...
            if (attachment.getUnits() <= 0) {
                throw new SbrException.NotValidException("Reserve claim number of units must be positive: " + attachment.getUnits());
            }
            CurrencyType.validate(Currency.getCurrency(attachment.getCurrencyId()), transaction);
        }
    ...
    public static final TransactionType CURRENCY_TRANSFER = new MonetarySystem() {
        ...
        void validateAttachment(Transaction transaction) throws SbrException.ValidationException {
            ...
            if (attachment.getUnits() <= 0) {
                throw new SbrException.NotValidException("Invalid currency transfer: " + attachment.getJSONObject());
            }
            if (transaction.getRecipientId() == Genesis.CREATOR_ID) {
                throw new SbrException.NotValidException("Currency transfer to genesis account not allowed");
            }
            Currency currency = Currency.getCurrency(attachment.getCurrencyId());
            CurrencyType.validate(currency, transaction);
            if (! currency.isActive()) {
                throw new SbrException.NotCurrentlyValidException("Currency not currently active: " + attachment.getJSONObject());
            }
        }
    ...
    public static final TransactionType PUBLISH_EXCHANGE_OFFER = new MonetarySystem() {
        ...
        void validateAttachment(Transaction transaction) throws SbrException.ValidationException {
            ...
            if (attachment.getBuyRateNQT() <= 0
                ...
                throw new SbrException.NotValidException(String.format("Invalid exchange offer, buy rate %d and sell rate %d has to be larger than 0, buy rate cannot be larger than sell rate",
                        attachment.getBuyRateNQT(), attachment.getSellRateNQT()));
            }
            if (attachment.getTotalBuyLimit() < 0
                ...
                throw new SbrException.NotValidException("Invalid exchange offer, units and height cannot be negative: " + attachment.getJSONObject());
            }
            if (attachment.getTotalBuyLimit() < attachment.getInitialBuySupply()
                ...
                throw new SbrException.NotValidException("Initial supplies must not exceed total limits");
            }
            if (attachment.getTotalBuyLimit() == 0 && attachment.getTotalSellLimit() == 0) {
                throw new SbrException.NotValidException("Total buy and sell limits cannot be both 0");
            }
            if (attachment.getInitialBuySupply() == 0 && attachment.getInitialSellSupply() == 0) {
                throw new SbrException.NotValidException("Initial buy and sell supply cannot be both 0");
            }
            if (attachment.getExpirationHeight() <= attachment.getFinishValidationHeight(transaction)) {
                throw new SbrException.NotCurrentlyValidException("Expiration height must be after transaction execution height");
            }
            Currency currency = Currency.getCurrency(attachment.getCurrencyId());
            CurrencyType.validate(currency, transaction);
            if (! currency.isActive()) {
    ...
    abstract static class MonetarySystemExchange extends MonetarySystem {
        ...
        final void validateAttachment(Transaction transaction) throws SbrException.ValidationException {
            ...
            if (attachment.getRateNQT() <= 0 || attachment.getUnits() == 0) {
                throw new SbrException.NotValidException("Invalid exchange: " + attachment.getJSONObject());
            }
            Currency currency = Currency.getCurrency(attachment.getCurrencyId());
            CurrencyType.validate(currency, transaction);
            if (! currency.isActive()) {
                throw new SbrException.NotCurrentlyValidException("Currency not active: " + attachment.getJSONObject());
            }
        }
    ...
    public static final TransactionType EXCHANGE_BUY = new MonetarySystemExchange() {
        ...
        void applyAttachment(Transaction transaction, Account senderAccount, Account recipientAccount) {
            ...
            CurrencyExchangeOffer.exchangeSBRForCurrency(transaction, senderAccount, attachment.getCurrencyId(), attachment.getRateNQT(), attachment.getUnits());
        }
    ...
    public static final TransactionType EXCHANGE_SELL = new MonetarySystemExchange() {
        ...
        void applyAttachment(Transaction transaction, Account senderAccount, Account recipientAccount) {
            Attachment.MonetarySystemExchangeSell attachment = (Attachment.MonetarySystemExchangeSell) transaction.getAttachment();
            ExchangeRequest.addExchangeRequest(transaction, attachment);
            CurrencyExchangeOffer.exchangeCurrencyForSBR(transaction, senderAccount, attachment.getCurrencyId(), attachment.getRateNQT(), attachment.getUnits());
        }
    ...
    public static final TransactionType CURRENCY_MINTING = new MonetarySystem() {
        ...
        void validateAttachment(Transaction transaction) throws SbrException.ValidationException {
            ...
            CurrencyType.validate(currency, transaction);
            if (attachment.getUnits() <= 0) {
                throw new SbrException.NotValidException("Invalid number of units: " + attachment.getUnits());
            }
            if (attachment.getUnits() > (currency.getMaxSupply() - currency.getReserveSupply()) / Constants.MAX_MINTING_RATIO) {
                throw new SbrException.NotValidException(String.format("Cannot mint more than 1/%d of the total units supply in a single request", Constants.MAX_MINTING_RATIO));
            }
            if (!currency.isActive()) {
                throw new SbrException.NotCurrentlyValidException("Currency not currently active " + attachment.getJSONObject());
            }
            ...
            if (attachment.getCounter() <= counter) {
                throw new SbrException.NotCurrentlyValidException(String.format("Counter %d has to be bigger than %d", attachment.getCounter(), counter));
            }
            if (!CurrencyMinting.meetsTarget(transaction.getSenderId(), currency, attachment)) {
                throw new SbrException.NotCurrentlyValidException(String.format("Hash doesn't meet target %s", attachment.getJSONObject()));
            }
        }
    ...
    public static final TransactionType CURRENCY_DELETION = new MonetarySystem() {
        ...
        void validateAttachment(Transaction transaction) throws SbrException.ValidationException {
            ...
            if (!currency.canBeDeletedBy(transaction.getSenderId())) {
                throw new SbrException.NotCurrentlyValidException("Currency " + Long.toUnsignedString(currency.getId()) + " cannot be deleted by account " +
