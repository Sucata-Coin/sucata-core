...
final class TransactionImpl implements Transaction {
    ...
    static final class BuilderImpl implements Builder {
        ...
        @Override
        public TransactionImpl build(String secretPhrase) throws SbrException.NotValidException {
            if (timestamp == Integer.MAX_VALUE) {
                timestamp = Sbr.getEpochTime();
            }
            if (!ecBlockSet) {
        ...
        @Override
        public TransactionImpl build() throws SbrException.NotValidException {
            return build(null);
        }
    ...
    private TransactionImpl(BuilderImpl builder, String secretPhrase) throws SbrException.NotValidException {
        ...
        if (builder.feeNQT <= 0 || (Constants.correctInvalidFees && builder.signature == null)) {
            int effectiveHeight = (height < Integer.MAX_VALUE ? height : Sbr.getBlockchain().getHeight());
        ...
        if (builder.signature != null && secretPhrase != null) {
            throw new SbrException.NotValidException("Transaction is already signed");
            ...
            if (getSenderPublicKey() != null && ! Arrays.equals(senderPublicKey, Crypto.getPublicKey(secretPhrase))) {
                throw new SbrException.NotValidException("Secret phrase doesn't match transaction sender public key");
            }
    ...
    static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes) throws SbrException.NotValidException {
        try {
            ...
            if (buffer.hasRemaining()) {
                throw new SbrException.NotValidException("Transaction bytes too long, " + buffer.remaining() + " extra bytes");
            }
            return builder;
        } catch (SbrException.NotValidException|RuntimeException e) {
            Logger.logDebugMessage("Failed to parse transaction bytes: " + Convert.toHexString(bytes));
            throw e;
        }
    }

    static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes, JSONObject prunableAttachments) throws SbrException.NotValidException {
    ...
    static TransactionImpl parseTransaction(JSONObject transactionData) throws SbrException.NotValidException {
        ...
        if (transaction.getSignature() != null && !transaction.checkSignature()) {
            throw new SbrException.NotValidException("Invalid transaction signature for transaction " + transaction.getJSONObject().toJSONString());
        }
        return transaction;
    }

    static TransactionImpl.BuilderImpl newTransactionBuilder(JSONObject transactionData) throws SbrException.NotValidException {
        try {
            ...
            if (transactionType == null) {
                throw new SbrException.NotValidException("Invalid transaction type: " + type + ", " + subtype);
            }
    ...
    @Override
    public void validate() throws SbrException.ValidationException {
        if (timestamp == 0 ? (deadline != 0 || feeNQT != 0) : (deadline < 1 || feeNQT <= 0)
                || feeNQT > Constants.MAX_BALANCE_NQT
                || amountNQT < 0
                || amountNQT > Constants.MAX_BALANCE_NQT
                || type == null) {
            throw new SbrException.NotValidException("Invalid transaction parameters:\n type: " + type + ", timestamp: " + timestamp
        ...
        if (referencedTransactionFullHash != null && referencedTransactionFullHash.length != 32) {
            throw new SbrException.NotValidException("Invalid referenced transaction full hash " + Convert.toHexString(referencedTransactionFullHash));
        }

        if (attachment == null || type != attachment.getTransactionType()) {
            throw new SbrException.NotValidException("Invalid attachment " + attachment + " for transaction of type " + type);
        }

        if (! type.canHaveRecipient()) {
            if (recipientId != 0 || getAmountNQT() != 0) {
                throw new SbrException.NotValidException("Transactions of this type must have recipient == 0, amount == 0");
            }
        }

        if (type.mustHaveRecipient()) {
            if (recipientId == 0) {
                throw new SbrException.NotValidException("Transactions of this type must have a valid recipient");
            }
        }
        ...
        for (Appendix.AbstractAppendix appendage : appendages) {
            ...
            if (! appendage.verifyVersion()) {
                throw new SbrException.NotValidException("Invalid attachment version " + appendage.getVersion());
            }
        ...
        if (getFullSize() > Constants.MAX_PAYLOAD_LENGTH) {
            throw new SbrException.NotValidException("Transaction size " + getFullSize() + " exceeds maximum payload size");
        }
        int blockchainHeight = Sbr.getBlockchain().getHeight();
        if (!validatingAtFinish) {
            long minimumFeeNQT = getMinimumFeeNQT(blockchainHeight);
            if (feeNQT < minimumFeeNQT) {
                throw new SbrException.NotCurrentlyValidException(String.format("Transaction fee %f %s less than minimum fee %f %s at height %d",
                        ((double) feeNQT) / Constants.ONE_SBR, Constants.COIN_SYMBOL, ((double) minimumFeeNQT) / Constants.ONE_SBR, Constants.COIN_SYMBOL, blockchainHeight));
            }
            if (ecBlockId != 0) {
                if (blockchainHeight < ecBlockHeight) {
                    throw new SbrException.NotCurrentlyValidException("ecBlockHeight " + ecBlockHeight
    ...
    private long getMinimumFeeNQT(int blockchainHeight) {
        ...
        if (referencedTransactionFullHash != null) {
            totalFee = Math.addExact(totalFee, Constants.ONE_SBR);
        }
        return totalFee;
    }
