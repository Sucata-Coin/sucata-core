...
import static sbr.http.JSONResponses.MISSING_ADMIN_PASSWORD;
import static sbr.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static sbr.http.JSONResponses.LOCKED_ADMIN_PASSWORD;
import static sbr.http.JSONResponses.NO_PASSWORD_IN_CONFIG;
...
public final class API {
    ...
    public static final String adminPassword = Sbr.getStringProperty("sbr.adminPassword", "", true);
    static final boolean disableAdminPassword;
    static final int maxRecords = Sbr.getIntProperty("sbr.maxAPIRecords");
    static final boolean enableAPIUPnP = Sbr.getBooleanProperty("sbr.enableAPIUPnP");
    public static final int apiServerIdleTimeout = Sbr.getIntProperty("sbr.apiServerIdleTimeout");
    public static final boolean apiServerCORS = Sbr.getBooleanProperty("sbr.apiServerCORS");
    private static final String forwardedForHeader = Sbr.getStringProperty("sbr.forwardedForHeader");
    ...
    static {
        List<String> disabled = new ArrayList<>(Sbr.getStringListProperty("sbr.disabledAPIs"));
        Collections.sort(disabled);
        disabledAPIs = Collections.unmodifiableList(disabled);
        disabled = Sbr.getStringListProperty("sbr.disabledAPITags");
        Collections.sort(disabled);
        ...
        List<String> allowedBotHostsList = Sbr.getStringListProperty("sbr.allowedBotHosts");
        ...
        boolean enableAPIServer = Sbr.getBooleanProperty("sbr.enableAPIServer");
        if (enableAPIServer) {
            final int port = Constants.isTestnet ? TESTNET_API_PORT : Sbr.getIntProperty("sbr.apiServerPort");
            final int sslPort = Constants.isTestnet ? TESTNET_API_SSLPORT : Sbr.getIntProperty("sbr.apiServerSSLPort");
            final String host = Sbr.getStringProperty("sbr.apiServerHost");
            disableAdminPassword = Sbr.getBooleanProperty("sbr.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            boolean enableSSL = Sbr.getBooleanProperty("sbr.apiSSL");
            //
            ...
            if (enableSSL) {
                ...
                String keyStorePath = Paths.get(Sbr.getUserHomeDir()).resolve(Paths.get(Sbr.getStringProperty("sbr.keyStorePath"))).toString();
                Logger.logInfoMessage("Using keystore: " + keyStorePath);
                sslContextFactory.setKeyStorePath(keyStorePath);
                sslContextFactory.setKeyStorePassword(Sbr.getStringProperty("sbr.keyStorePassword", null, true));
                ...
                sslContextFactory.setKeyStoreType(Sbr.getStringProperty("sbr.keyStoreType"));
                List<String> ciphers = Sbr.getStringListProperty("sbr.apiSSLCiphers");
                if (!ciphers.isEmpty()) {
            ...
            String apiResourceBase = Sbr.getStringProperty("sbr.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{Sbr.getStringProperty("sbr.apiWelcomeFile")});
            }

            String javadocResourceBase = Sbr.getStringProperty("sbr.javadocResourceBase");
            ...
            ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/sbr");
            servletHolder.getRegistration().setMultipartConfig(new MultipartConfigElement(
                    null, Math.max(Sbr.getIntProperty("sbr.maxUploadFileSize"), Constants.MAX_TAGGED_DATA_DATA_LENGTH), -1L, 0));

            servletHolder = apiHandler.addServlet(APIProxyServlet.class, "/sbr-proxy");
            ...
            if (!Sbr.getBooleanProperty("sbr.enableAPIServerGZIPFilter", isOpenAPI)) {
                gzipHandler.setExcludedPaths("/sbr", "/sbr-proxy");
            }
            gzipHandler.setIncludedMethods("GET", "POST");
            gzipHandler.setMinGzipSize(sbr.peer.Peers.MIN_COMPRESS_SIZE);
            ...
            if (Sbr.getBooleanProperty("sbr.apiFrameOptionsSameOrigin")) {
