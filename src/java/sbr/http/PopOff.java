...
public final class PopOff extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        try {
            Sbr.getBlockchainProcessor().setGetMoreBlocks(false);
            if (numBlocks > 0) {
                blocks = Sbr.getBlockchainProcessor().popOffTo(Sbr.getBlockchain().getHeight() - numBlocks);
            } else if (height > 0) {
                blocks = Sbr.getBlockchainProcessor().popOffTo(height);
            } else {
                return JSONResponses.missing("numBlocks", "height");
            }
        } finally {
            Sbr.getBlockchainProcessor().setGetMoreBlocks(true);
        }
        ...
        if (keepTransactions) {
            blocks.forEach(block -> Sbr.getTransactionProcessor().processLater(block.getTransactions()));
        }
        return response;
    }
