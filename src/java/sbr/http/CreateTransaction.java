...
abstract class CreateTransaction extends APIServlet.APIRequestHandler {
    ...
    private Appendix.Phasing parsePhasing(HttpServletRequest req) throws ParameterException {
        int finishHeight = ParameterParser.getInt(req, "phasingFinishHeight",
                Sbr.getBlockchain().getHeight() + 1,
                Sbr.getBlockchain().getHeight() + Constants.MAX_PHASING_DURATION + 1,
                true);
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId,
                                            long amountNQT, Attachment attachment) throws SbrException {
        ...
        if (ecBlockId != 0 && ecBlockId != Sbr.getBlockchain().getBlockIdAtHeight(ecBlockHeight)) {
            return INCORRECT_EC_BLOCK;
        }
        if (ecBlockId == 0 && ecBlockHeight > 0) {
            ecBlockId = Sbr.getBlockchain().getBlockIdAtHeight(ecBlockHeight);
        }
        ...
        try {
            Transaction.Builder builder = Sbr.newTransactionBuilder(publicKey, amountNQT, feeNQT,
            ...
            } catch (SbrException.NotYetEncryptedException ignore) {}
        ...
        } catch (SbrException.NotYetEnabledException e) {
