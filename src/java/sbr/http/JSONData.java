...
import sbr.AccountRestrictions;
import sbr.Alias;
import sbr.Appendix;
import sbr.Asset;
import sbr.AssetDelete;
import sbr.AssetDividend;
import sbr.AssetTransfer;
import sbr.Attachment;
import sbr.Block;
import sbr.Constants;
import sbr.Currency;
import sbr.CurrencyExchangeOffer;
import sbr.CurrencyFounder;
import sbr.CurrencyTransfer;
import sbr.CurrencyType;
import sbr.DigitalGoodsStore;
import sbr.Exchange;
import sbr.ExchangeRequest;
import sbr.FundingMonitor;
import sbr.Generator;
import sbr.HoldingType;
import sbr.MonetarySystem;
import sbr.Sbr;
import sbr.Order;
import sbr.PhasingPoll;
import sbr.PhasingVote;
import sbr.Poll;
import sbr.PrunableMessage;
import sbr.Shuffler;
import sbr.Shuffling;
import sbr.ShufflingParticipant;
import sbr.TaggedData;
import sbr.Token;
import sbr.Trade;
import sbr.Transaction;
import sbr.Vote;
import sbr.VoteWeighting;
import sbr.crypto.Crypto;
import sbr.crypto.EncryptedData;
import sbr.db.DbIterator;
import sbr.peer.Hallmark;
...
public final class JSONData {
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance) {
        return accountBalance(account, includeEffectiveBalance, Sbr.getBlockchain().getHeight());
    }
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance, int height) {
        ...
        if (account == null) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceSBR", "0");
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceSBR", account.getEffectiveBalanceSBR(height));
    ...
    static JSONObject lessor(Account account, boolean includeEffectiveBalance) {
        ...
        if (accountLease.getCurrentLesseeId() != 0) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceSBR", String.valueOf(account.getGuaranteedBalanceNQT() / Constants.ONE_SBR));
            }
        }
    ...
    static JSONObject shuffling(Shuffling shuffling, boolean includeHoldingInfo) {
        ...
        if (includeHoldingInfo && shuffling.getHoldingType() != HoldingType.SBR) {
        ...
        if (includeExecutedPhased) {
            ...
            try (DbIterator<PhasingPoll.PhasingPollResult> phasingPollResults = PhasingPoll.getApproved(block.getHeight())) {
                for (PhasingPoll.PhasingPollResult phasingPollResult : phasingPollResults) {
                    ...
                    if (includeTransactions) {
                        phasedTransactions.add(transaction(Sbr.getBlockchain().getTransaction(phasedTransactionId)));
                    } else {
    ...
    static JSONObject pollResults(Poll poll, List<Poll.OptionResult> results, VoteWeighting voteWeighting) {
        ...
        if (voteWeighting.getMinBalanceModel() == VoteWeighting.MinBalanceModel.ASSET) {
            ...
            if (currency != null) {
                ...
                Transaction currencyIssuance = Sbr.getBlockchain().getTransaction(voteWeighting.getHoldingId());
    ...
    static JSONObject transaction(Transaction transaction, Filter<Appendix> filter) {
        ...
        json.put("confirmations", Sbr.getBlockchain().getHeight() - transaction.getHeight());
    ...
    private static void putExpectedTransaction(JSONObject json, Transaction transaction) {
        json.put("height", Sbr.getBlockchain().getHeight() + 1);
        json.put("phased", transaction.getPhasing() != null);
    ...
    static void ledgerEntry(JSONObject json, LedgerEntry entry, boolean includeTransactions, boolean includeHoldingInfo) {
        ...
        if (includeTransactions && entry.getEvent().isTransaction()) {
            Transaction transaction = Sbr.getBlockchain().getTransaction(entry.getEventId());
