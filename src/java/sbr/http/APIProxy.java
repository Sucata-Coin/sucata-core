...
public class APIProxy {
    ...
    static final boolean enableAPIProxy = Constants.isLightClient ||
            (Sbr.getBooleanProperty("sbr.enableAPIProxy") && ! API.isOpenAPI);
    private static final int blacklistingPeriod = Sbr.getIntProperty("sbr.apiProxyBlacklistingPeriod") / 1000;
    static final String forcedServerURL = Sbr.getStringProperty("sbr.forceAPIProxyServerURL", "");
    ...
    static boolean isActivated() {
        return Constants.isLightClient || (enableAPIProxy && Sbr.getBlockchainProcessor().isDownloading());
    }
    ...
    boolean blacklistHost(String host) {
        ...
        blacklistedPeers.put(host, Sbr.getEpochTime() + blacklistingPeriod);
