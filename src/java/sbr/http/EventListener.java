...
import sbr.AccountLedger;
import sbr.AccountLedger.LedgerEntry;
import sbr.Block;
import sbr.BlockchainProcessor;
import sbr.Db;
import sbr.Sbr;
import sbr.Transaction;
import sbr.TransactionProcessor;
 ...
 * has not been received within sbr.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by sbr.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = Sbr.getIntProperty("sbr.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(Sbr.getIntProperty("sbr.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = Sbr.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = Sbr.getTransactionProcessor();
    ...
    /** Sbr event listeners */
    private final List<SbrEventListener> sbrEventListeners = new ArrayList<>();
     ...
     *
     * Sbr event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of Sbr event registrations
     ...
     *
     * @param   eventRegistrations      Sbr event registrations
     * @throws  EventListenerException  Invalid Sbr event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<SbrEventListener> it = sbrEventListeners.iterator();
                while (it.hasNext()) {
                    SbrEventListener listener = it.next();
                ...
                if (addListener) {
                    SbrEventListener listener = new SbrEventListener(event);
                    listener.addListener();
                    sbrEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            sbrEventListeners.forEach(SbrEventListener::removeListener);
        } finally {
            lock.unlock();
        }
     ...
     * Sbr event listener
     */
    private class SbrEventListener {

        /** Event handler */
        private final SbrEventHandler eventHandler;

        /**
         * Create the Sbr event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public SbrEventListener(EventRegistration eventRegistration) throws EventListenerException {
        ...
        /**
         * Add the Sbr listener for this event
         */
        ...
        /**
         * Remove the Sbr listener for this event
         */
        ...
        /**
         * Return the hash code for this Sbr event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two Sbr events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof SbrEventListener) &&
                    eventHandler.equals(((SbrEventListener)obj).eventHandler));
        }

        /**
         * Sbr listener event handler
         */
        private abstract class SbrEventHandler {
            ...
            /**
             * Create the Sbr event handler
             *
             * @param   eventRegistration   Event registration
             */
            public SbrEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof SbrEventHandler) &&
                                        owner == ((SbrEventHandler)obj).owner &&
                                        accountId == ((SbrEventHandler)obj).accountId &&
                                        event == ((SbrEventHandler)obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends SbrEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends SbrEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends SbrEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends SbrEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
         ...
         *
         * @param   event           Sbr listener event
        ...
        /**
         * Return the Sbr listener event
         *
         * @return                  Sbr listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }
