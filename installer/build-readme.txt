Building the SBR installer

Note: all pathes are relative to the git repository folder (sbr-private)
...
Test X11 installer: java -jar sbr-client-<version>.jar
Test console installer: java -jar sbr-client-<version>.jar -console
...
Edit mac-release-package.sh and replace "Developer ID Application: Stichting SBR (YU63QW5EFW)" with your developer signature (you can also remove -Bmac.signing-key-developer-id-app="Developer ID Application: Stichting SBR (YU63QW5EFW)" to generate an unsigned package)
...
Copy the content of Java JRE folder (C:\Program Files\Java\jre<version>) into the jre folder under your sbr-private git repository preserving the folder structure
...
Test the installer using Java: java -jar sbr-client-<version>.jar
Test the windows executable: sbr-client-<version>.exe
...
sbr.jar is now self executing, the full classpath is embedded into the jar file manifest automatically by jar.sh
