...
final class ConfigHandler {
    ...
    private static final String FILE_HEADER =
            ...
            "# See conf/sbr-default.properties for a full list\n#\n\n";
    private static final String JAR = "sbr.jar";
    private static final String SERVER = "http://localhost";
    private static final int[] PORTS = { 6876, 7876 };   // try to detect both testnet and real servers
    private static final String VAR_PREFIX = "sbr.installer.";

    public static final String FILE_PATH = "conf/sbr-installer.properties";
    ...
    public boolean shutdownServer() {
        ...
        for (int port: PORTS) {
            try {
                URL url = new URL(SERVER + ':' + port + "/sbr?requestType=shutdown");
                HttpURLConnection conn = (HttpURLConnection) url.openConnection();
    ...
    public boolean isSbrInstallDir(String path) {
        return path != null && Files.exists(Paths.get(path, JAR));
    }

    public boolean cleanSbrInstallDir(String installPath, boolean retry) {
        if (isSbrInstallDir(installPath)) {
