...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.listAccounts = function() {
        ...
        loginAccount.empty();
		if (NRS.getStrItem("savedSbrAccounts") && NRS.getStrItem("savedSbrAccounts") != ""){
			$('#login_account_container').show();
			$('#login_account_container_other').hide();
			var accounts = NRS.getStrItem("savedSbrAccounts").split(";");
    ...
    $("#loginButtons").find(".btn").click(function (e) {
        ...
        if (type == "account") {
            ...
            NRS.scanQRCode(readerId, function(text) {
                var sbrAddress = new SbrAddress();
                if (sbrAddress.set(text)) {
	...
	NRS.removeAccount = function(account) {
		var accounts = NRS.getStrItem("savedSbrAccounts").replace(account+';','');
		if (accounts == '') {
			NRS.removeItem('savedSbrAccounts');
		} else {
			NRS.setStrItem("savedSbrAccounts", accounts);
		}
		NRS.listAccounts();
	};

    function rememberAccount(account) {
        var accountsStr = NRS.getStrItem("savedSbrAccounts");
        if (!accountsStr) {
            NRS.setStrItem("savedSbrAccounts", account + ";");
            return;
        }
        ...
        NRS.setStrItem("savedSbrAccounts", accountsStr + account + ";");
    }
			...
			NRS.sendRequest(accountRequest, requestVariable, function(response, data) {
				...
				NRS.sendRequest("getAccountPublicKey", {
					...
					if (NRS.getStrItem("savedSbrAccounts") && NRS.getStrItem("savedSbrAccounts")!=""){
						...
						accounts = NRS.getStrItem("savedSbrAccounts").split(";");
						$.each(accounts, function(index, account) {
							if (account != ''){
	...
	NRS.initPluginWarning = function() {
		if (NRS.activePlugins) {
			...
			html += "Plugins are not sandboxed or restricted in any way and have full accesss to your client system including your Sbr passphrase.";
			html += "</p>";
