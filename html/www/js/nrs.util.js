...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.calculateOrderPricePerWholeQNT = function (price, decimals, returnAsObject) {
		...
		return NRS.convertToSBR(price.multiply(new BigInteger("" + Math.pow(10, decimals))), returnAsObject);
	};
    ...
    NRS.calculateOrderTotal = function (quantityQNT, priceNQT) {
        return NRS.convertToSBR(calculateOrderTotalImpl(quantityQNT, priceNQT));
    };
    ...
    NRS.convertToSBR = function (amount, returnAsObject) {
        var negative = "";
        var mantissa = "";
    ...
    NRS.formatAmount = function (amount, round, no_escaping, zeroPad) {
        ...
        if (typeof amount == "object") {
            var params = NRS.convertToSBR(amount, true);
        ...
        if (NRS.settings) {
            ...
            var maxLength = parseInt(NRS.settings.max_sbr_decimals) + offset;
    ...
    NRS.isRsAccount = function(account) {
        return NRS.isRsAccountImpl(account, NRS.constants.ACCOUNT_RS_MATCH ? NRS.constants.ACCOUNT_RS_MATCH : NRS.getRsAccountRegex("SBR"));
    };
    ...
    NRS.getAccountMask = function(c) {
        switch(c) {
            ...
            default:
                return NRS.constants.ACCOUNT_MASK_PREFIX ? NRS.constants.ACCOUNT_MASK_PREFIX : "SBR-";
        }
    };

    NRS.printPaperWallet = function(passphrase) {
        var $pageHeader = $("<h2 data-i18n='sbr_ardor_paper_wallet'>SBR Paper Wallet</h2>");
