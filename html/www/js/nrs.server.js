...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.sendRequest = function (requestType, data, callback, options) {
        ...
        try {
            var sbrFields = [
                ...
                ["priceSBR", "priceNQT"],
                ["refundSBR", "refundNQT"],
                ["discountSBR", "discountNQT"],
                ["phasingQuorumSBR", "phasingQuorum"],
                ["phasingMinBalanceSBR", "phasingMinBalance"],
                ["controlQuorumSBR", "controlQuorum"],
                ["controlMinBalanceSBR", "controlMinBalance"],
                ["controlMaxFeesSBR", "controlMaxFees"],
                ["minBalanceSBR", "minBalance"],
                ["shufflingAmountSBR", "amount"],
                ["monitorAmountSBR", "amount"],
                ["monitorThresholdSBR", "threshold"]
            ];

            for (i = 0; i < sbrFields.length; i++) {
                var sbrField = sbrFields[i][0];
                var nqtField = sbrFields[i][1];
                if (sbrField in data) {
                    data[nqtField] = NRS.convertToNQT(data[sbrField]);
                    delete data[sbrField];
                }
            }
        } catch (err) {
            callback({
            ...
            if (maxFees > 0 && new BigInteger(data.feeNQT).compareTo(new BigInteger(phasingControl.maxFees)) > 0) {
                callback({
                    ...
                    "errorDescription": $.t("error_fee_exceeds_max_account_control_fee", {
                        "maxFee": NRS.convertToSBR(phasingControl.maxFees), "symbol": NRS.constants.COIN_SYMBOL
                    })
                });
                return;
            }
    ...
    NRS.processAjaxRequest = function (requestType, data, callback, options) {
        ...
        if (options.remoteNode) {
            url = options.remoteNode.getUrl() + "/sbr";
        } else {
