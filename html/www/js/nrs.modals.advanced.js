...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.initAdvancedModalFormValues = function($modal) {
		...
		context = {
			...
			inputName: "phasingQuorumSBR",
			addonText: "SBR",
			addonI18n: "sbr_unit"
		};
		$elems = NRS.initModalUIElement($modal, '.approve_transaction_amount_sbr', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		context['inputName'] = 'phasingMinBalanceSBR';
		context['addonText'] = 'SBR';
		context['addonI18n'] = 'sbr_unit';
		$elems = NRS.initModalUIElement($modal, '.approve_min_balance_sbr', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
