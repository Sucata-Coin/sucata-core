...
var NRS = (function(NRS, $) {
    ...
    NRS.submitForm = function($modal, $btn) {
		...
		$form.find(":input").each(function() {
			if ($(this).is(":invalid")) {
				var error = "";
				var name = String($(this).attr("name")).replace("SBR", "").replace("NQT", "").capitalize();
				var value = $(this).val();
		...
		if (!NRS.showedFormWarning) {
			if ("amountSBR" in data && NRS.settings["amount_warning"] && NRS.settings["amount_warning"] != "0") {
				try {
					var amountNQT = NRS.convertToNQT(data.amountSBR);
				} catch (err) {
			...
			if ("feeSBR" in data && NRS.settings["fee_warning"] && NRS.settings["fee_warning"] != "0") {
				try {
					var feeNQT = NRS.convertToNQT(data.feeSBR);
				} catch (err) {
			...
			var convertSBRFields = ["phasingQuorumSBR", "phasingMinBalanceSBR"];
			$.each(convertSBRFields, function(key, field) {
				if (field in data) {
					try {
    ...
    function updateFee(modal, feeNQT) {
        ...
        fee.val(NRS.convertToSBR(feeNQT));
        var recalcIndicator = $("#" + modal.attr('id').replace('_modal', '') + "_recalc");
