...
var NRS = (function(NRS, $) {
    ...
    $("#dividend_payment_amount_per_share, #dividend_payment_height").on("blur", function() {
        ...
        var amountSBRPerShare = $modal.find("#dividend_payment_amount_per_share").val();
        ...
        if (amountSBRPerShare && /^\d+$/.test(height)) {
            NRS.getAssetAccounts(NRS.getCurrentAsset().asset, height,
                function (response) {
                    ...
                    var priceNQT = new BigInteger(NRS.calculatePricePerWholeQNT(NRS.convertToNQT(amountSBRPerShare), NRS.getCurrentAsset().decimals));
                    var totalSBR = NRS.calculateOrderTotal(totalQuantityQNT, priceNQT);
